...
export const  useGetDS = () => {
    ...
    const fetchAllData = async () => {
        try {
            const response = await fetch('http://infosafe.live:8080/api/datascope/getDs', {
                method: "GET",
                headers: {
    ...
    const fetchMyData = async () => {
        try {
            const response = await fetch('http://infosafe.live:8080/api/datascope/getMyDatascopes', {
                method: "GET",
                headers: {
                    Authorization: "Bearer " + sessionStorage.getItem('accessToken')
                }
