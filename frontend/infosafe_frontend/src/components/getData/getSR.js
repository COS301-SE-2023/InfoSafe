...
export const useGetSR = () => {
    ...
    const fetchAllData = async () => {
        try {
            const response = await fetch('http://infosafe.live:8080/api/supportrequest/getSr', {
                method: "GET",
                headers: {
    ...
    const fetchMyData = async () => {
        try {
            const response = await fetch('http://infosafe.live:8080/api/supportrequest/getSrById', {
                method: "GET",
                headers: {
                    Authorization: "Bearer " + sessionStorage.getItem('accessToken')
                }
